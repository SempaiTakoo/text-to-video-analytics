...
def render_column_description(column: sqlalchemy.Column[Any]) -> str:
    ...
    comment = column.comment or "No description"
    return f"- {column.name} ({column.type}{pk}{fk}) - {comment}"


def render_table_description(table: sqlalchemy.Table) -> str:
    header = f"Table: {table.name} — {table.comment or 'No description'}"
    columns = "\n".join(
        render_column_description(column) for column in table.columns
    )
    return f"{header}\nColumns:\n{columns}"
...
db_schema = render_db_description(metadata)
